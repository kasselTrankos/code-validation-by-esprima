CNT.ngModule.controller('undertakeCPTrendAnalysisController', [
	...
	'$parse',
	'$filter',
	function (undertakeCPTrendAnalysisService, $stateParams, $scope, gettextCatalog, T3_CabeceraPresentacionService, T3_CommunicationService, T3_TrazaService, T3_StateService, T3_LanguageService, PopupService, $state, T3_StorageService, $parse, $filter) {
		$scope.init = function () {
			T3_StateService.init($scope, undertakeCPTrendAnalysisService, { CGT_UndertakeCPTrendAnalysis_IN: $stateParams.cgtUndertakeCPTrendAnalysisIn || $scope.cgtUndertakeCPTrendAnalysisIn });
			...
			$scope.tablaProblemasCliente = { paginacion: {} };
			$scope.searchCustomerProblemTable = {
				datos: [],
				metadatos: { numeroPagina: 0 }
			};
			if ((!$stateParams.cgtUndertakeCPTrendAnalysisIn || $stateParams.cgtUndertakeCPTrendAnalysisIn === void 0) && (!$scope.cgtUndertakeCPTrendAnalysisIn || $scope.cgtUndertakeCPTrendAnalysisIn === void 0)) {
		...
		$scope.setStateData = function (data, partyRole) {
			...
			for (i; i < l; i++) {
				var pattern = $filter('filter')($scope.patternSate, { to: _generate[i].node })[0];
		...
		$scope.onListadoCuentas_OUT = function (event, data) {
			data = { customerAccount: [{ id: 1.222222222232233e+23 }] };
		...
		$scope.updateCustomerId = function (data) {
			...
			for (i; i < l; i++) {
				if (Number(data.party.partyRoles[i].partyRoleSpecification.id) === 1 || Number(data.party.partyRoles[i].partyRoleSpecification.id) === 3) {
					$scope.CG_searchPartyRole_OUT.party = { partyRole: { id: data.party.partyRoles[i].id } };
					var dataUndertake = T3_StorageService.getItem({ cgName: 'undertakeCPAnalysisState' });
					if (dataUndertake) {
		...
		$scope.verDetalle = function (problema) {
			var cgtIN = {
				customerProblem: { id: problema.id },
				prevState: $state.current
			};
			$state.go('showDetailOfCPReport', { cgtShowDetailOfCPReportIn: cgtIN });
		};
		...
		$scope.limpiarForm = function () {
          ...
          { id: 3 }
			...
			T3_StorageService.delItem({ cgName: 'getCustomerServiceSuppEnquiry' });
		};
		...
		$scope.manageCustomerAccount = function () {
			var param = {
				cgtManageCustomerAccountIn: {
					customer: { id: $scope.cgtUndertakeCPTrendAnalysisIn.customer ? $scope.cgtUndertakeCPTrendAnalysisIn.customer.id : $scope.CG_searchPartyRole_OUT.party.partyRole.id },
					action: 2
				}
			};
		...
		$scope.getCustomerData_IN = function () {
			var _return = {};
			if ($scope.cgtUndertakeCPTrendAnalysisIn && $scope.cgtUndertakeCPTrendAnalysisIn.customer && $scope.cgtUndertakeCPTrendAnalysisIn.customer.id) {
				_return.customers = [{ id: $scope.cgtUndertakeCPTrendAnalysisIn.customer.id }];
			}
			if ($scope.CG_searchPartyRole_OUT.party && $scope.CG_searchPartyRole_OUT.party.partyRole && $scope.CG_searchPartyRole_OUT.party.partyRole.id) {
				_return.customers = [{ id: $scope.CG_searchPartyRole_OUT.party.partyRole.id }];
			}
			return _return;
		};
		$scope.getCustomerData = function () {
			undertakeCPTrendAnalysisService.getCustomerData($scope.getCustomerData_IN()).then(function (data) {
		...
		$scope.listCustomerProblemPhasePublic = function () {
			...
			$scope.openSpinner();
			undertakeCPTrendAnalysisService.listCustomerProblemPhasePublic(ListCustomerProblemPhasePublic_IN, ['AD_listCustomerProblemType_nameAsc_DSC']).then(function (data) {
				$scope.closeSpinner();
		...
		$scope.listCustomerProblemSituationPublic = function () {
			...
			$scope.openSpinner();
			undertakeCPTrendAnalysisService.listCustomerProblemSituationPublic(ListCustomerProblemSituationPublic_IN, ['AD_listCustomerProblemType_nameAsc_DSC']).then(function (data) {
				$scope.closeSpinner();
		...
		$scope.listCustomerProblemTypePublic = function () {
			...
			$scope.openSpinner();
			undertakeCPTrendAnalysisService.listCustomerProblemTypePublic(ListCustomerProblemTypePublic_IN, ['AD_listCustomerProblemType_nameAsc_DSC']).then(function (data) {
				$scope.closeSpinner();
		...
		$scope.listEntryTypePublic = function () {
			...
			$scope.openSpinner();
			undertakeCPTrendAnalysisService.listEntryTypePublic(ListEntryTypePublic_IN).then(function (data) {
				$scope.closeSpinner();
		...
		$scope.searchCustomerProblem_IN = function () {
			...
			if ($scope.cgtUndertakeCPTrendAnalysisIn.customer && !$scope.CG_searchPartyRole_OUT.party) {
				data.partyRole = { id: $scope.cgtUndertakeCPTrendAnalysisIn.customer.id };
			} else if (!$scope.cgtUndertakeCPTrendAnalysisIn.customer && $scope.CG_searchPartyRole_OUT.party) {
				data.partyRole = { id: $scope.CG_searchPartyRole_OUT.party.partyRole.id };
			} else if ($scope.cgtUndertakeCPTrendAnalysisIn.customer && $scope.CG_searchPartyRole_OUT.party) {
				data.partyRole = { id: $scope.CG_searchPartyRole_OUT.party.partyRole.id };
			}
			if ($scope.undertakeCPTrendAnalysisData.partyRole && $scope.undertakeCPTrendAnalysisData.partyRole.name) {
			...
			if ($scope.undertakeCPTrendAnalysisView.situacionProblema) {
				data.cpsituation = { 'id': $scope.undertakeCPTrendAnalysisView.situacionProblema };
			}
			if ($scope.undertakeCPTrendAnalysisView.numeroFactura) {
				data.customerBill = data.customerBill || {};
				data.customerBill = { 'billNo': $scope.undertakeCPTrendAnalysisView.numeroFactura };
			}
			if ($scope.undertakeCPTrendAnalysisView.codigo) {
				data.customerProblem = { 'code': $scope.undertakeCPTrendAnalysisView.codigo };
			}
			...
			if ($scope.undertakeCPTrendAnalysisView.faseProblema) {
				data.customerProblemPhase = { 'id': $scope.undertakeCPTrendAnalysisView.faseProblema };
			}
			...
			if ($scope.undertakeCPTrendAnalysisView.tipoProblema) {
				data.customerProblemType = { 'id': $scope.undertakeCPTrendAnalysisView.tipoProblema };
			}
			if ($scope.undertakeCPTrendAnalysisView.cuentaClienteId) {
				data.customerAccount = { 'id': $scope.undertakeCPTrendAnalysisView.cuentaClienteId };
			}
			if ($scope.undertakeCPTrendAnalysisView.startDate) {
				data.dateRange = { 'startDateTime': $scope.undertakeCPTrendAnalysisView.startDate };
			...
			if ($scope.undertakeCPTrendAnalysisView.problemaCliente) {
				data.entryType = { 'id': $scope.undertakeCPTrendAnalysisView.problemaCliente };
			}
		...
		$scope.searchCustomerProblem = function (SearchCustomerProblem_IN) {
			...
			undertakeCPTrendAnalysisService.searchCustomerProblem(SearchCustomerProblem_IN, [$scope.reordenarTabla()], $scope.getNumPageSearchCustomerProblem()).then(function (data) {
				$scope.closeSpinner();
				if (data) {
				...
				if ($scope.errores.mensaje2 === 'PRCL-E-00002') {
					...
					$scope.error(gettextCatalog.getString('undertakeCPTrendAnalysis-titulo'), gettextCatalog.getString('undertakeCPTrendAnalysis-PRCL-M-00013-title'), gettextCatalog.getString('undertakeCPTrendAnalysis-PRCL-M-00013-sub-title'));
					$scope.SearchCustomerProblem_OUT = [];
				} else {
					$scope.error();
				}
...
CNT.ngModule.controller('undertakeCPTrendAnalysisCtrl', [
	'$scope',
	'$modalInstance',
