...
CNT.ngModule.controller('categorizationFulfillmentCPController', [
	...
	function($scope, $q, $log, PopupService, gettextCatalog, categorizationFulfillmentCPService,
		...
		$scope.getSubscribes = function() {
            // PASO 7.1
			T3_CommunicationService.subscribe($scope, 'showCustomerOrderHistoryOH', function(event, data) {
				...
				$scope.EGT_ShowCustomerOrderHistory = data;
			});
		};
		...
		$scope.cargarDatosInicio = function() {
			...
			if (($scope.CGT_CategorizationFulfillmentCP_IN.customerProblemTaskType.id === categorizationFulfillmentCPConstant.CUSTOMERPROBLEMTASKTYPE_CODE.OPEN) ||
                ($scope.CGT_CategorizationFulfillmentCP_IN.customerProblemTaskType.id === categorizationFulfillmentCPConstant.CUSTOMERPROBLEMTASKTYPE_CODE.REOPEN)) {
                // PASO 1.1
				$scope.cgtShowCustomerOrderHistory($scope.CGT_CategorizationFulfillmentCP_IN.customerOrderWithProblem);
			}
		...
		$scope.checkCustomerOrder = function(dataIn) {
			var customerOrderOK = false;

			angular.forEach(dataIn.customerOrder, function(item) {
                // Comprobación de 'customerOrderItem'
				if (item.customerOrderItem) {
					var customerOrderItems = $filter('filter')(item.customerOrderItem, {
						id: null
					});
		...
		$scope.cgtShowCustomerOrderHistory = function(customerOrders) {
			...
			if ($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder &&
                ($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder.length > 0) &&
                $scope.CGT_CategorizationFulfillmentCP_IN.customerOrder[0]) {
				$scope.cgtShowCustomerOrderHistoryOHIn.customerOrder = {
					id: $scope.CGT_CategorizationFulfillmentCP_IN.customerOrder[0].id
				};
			}

			if($scope.CGT_CategorizationFulfillmentCP_IN.customerOrderWithProblem &&
                $scope.CGT_CategorizationFulfillmentCP_IN.customerOrderWithProblem.length > 0) {
				$scope.cgtShowCustomerOrderHistoryOHIn.customerOrderWithProblem = $scope.CGT_CategorizationFulfillmentCP_IN.customerOrderWithProblem;
			}

            // Comprobar si se viene de 'PASO 2.4.1' (customerOrderItems recuperados con 'OP_getFulfillmentCustomerProblemDataByCP')
			if (customerOrders) {
				ordersAndItems = $scope.getCustomerOrdersAndItems(customerOrders);
			} else {
                // Comprobar si viene informado 'customerOrderItem'
				if ($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder && ($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder.length > 0)) {
					ordersAndItems = $scope.getCustomerOrdersAndItems($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder);
				}
			}
		...
		$scope.getCustomerOrdersAndItems = function(customerOrderList) {
			var customerOrdersAndItems = [];

			angular.forEach(customerOrderList, function(customerOrderElement) {
				var order = {
					id: customerOrderElement.id
				};

                // Comprobar si viene informado 'customerOrderItem'
				if (customerOrderElement.customerOrderItems && (customerOrderElement.customerOrderItems.length > 0)) {
					order.customerOrderItem = [];

					angular.forEach(customerOrderElement.customerOrderItems, function(orderItem) {
						var item = {
							id: orderItem.id
						};
		...
		$scope.continuar = function() {
			...
			if (($scope.CGT_CategorizationFulfillmentCP_IN.customerProblemTaskType.id === categorizationFulfillmentCPConstant.CUSTOMERPROBLEMTASKTYPE_CODE.MODIFICATION) &&
                ($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder !== null) &&
                ($scope.CGT_CategorizationFulfillmentCP_IN.customerOrder !== undefined)) {

				if ($scope.EGT_ShowCustomerOrderHistory) {
                    // EGT_ShowCustomerOrderHistory.CustomerOrder_DTO_OUT[]
					angular.forEach($scope.EGT_ShowCustomerOrderHistory, function(customerOrder) {
						var order = {
							id: customerOrder.id,
						...
						angular.forEach(customerOrder.customerOrderItem, function(coItem) {

                            // Comprobar si existe el item y tiene motivo y submotivo
							var foundItem = $scope.getExistsItem(coItem.id);

							if (foundItem !== null) {
								coItem.cPMotive = foundItem.cpMotiveAbstract;
							}

							order.customerOrderItem.push(coItem);
						});

						EGT_CategorizationFulfillmentCPOK.customerOrder.push(order);
					});
				}
			} else {
                // EGT_ShowCustomerOrderHistory.CustomerOrder_DTO_OUT[]
			...
			for (var i = 0;
				...
				for (var j = 0;
					...
					if (existingItem.length > 0) {
						angular.forEach(existingItem[0].cOIDisputed, function(coiDisputed) {
	...
	cgtShowDetailOfCPReportIn: cgtShowDetailOfCPReportIn
		...
		$scope.getFulfillmentCustomerProblemDataByCP = function() {
			...
			var GetFulfillmentCustomerProblemDataByCP_IN = {
				...
				customerOrders: $scope.CGT_CategorizationFulfillmentCP_IN.customerOrder
			};
	...
	$scope.cgtShowCustomerOrderHistory(data.customerOrders);
},
                function(error) {
	PopupService.CloseSpinner();
