...
var validLoadModule = function(node, rules, callback){
    ...
    if(m0.length>0){
	    ...
	    for(var t =0; t<rules.loadModule.length; t++){
	    	isInArr(rules.loadModule[t], _act, function(status, elm){
	    		if(status===false){
	    			_err.push({
	...
	eswalk(ast, function(node, parent) {
		...
		if(!rules.sticky){
			if(node.type=='Property' &&
				...
				if(node.value.value!=rules.sticky){
					_err.push({
						title: `Sobra quitalo sticky !!!!`,
						loc: ast.body[0].expression.arguments[0].loc
					});
				}
			}
		}
		if(!rules.deepStateRedirect){
			if(node.type=='Property' &&
				node.key &&
				node.key.name =='deepStateRedirect'){
				if(node.value.value!=rules.deepStateRedirect){
					_err.push({
						title: `Sobra quitalo deepStateRedirect !!!!`,
						loc: node.loc
					});
...
var validLoadCommonComponent = function(node, rules, callback){
    ...
    if(m0 &&
    	...
    	for(var t =0; t<rules.loadCommonComponent.length; t++){
    		isInArr(rules.loadCommonComponent[t], _act, function(response, elm){
    			if(response===false){
    				_err.push({
						title: `Falta de añadir el module: "${rules.loadCommonComponent[t]}"`,
						loc: (elm) ? elm.loc : node.loc
					});
    			}
...
var isInArr = function(item, arr, callback){
	for(var i = 0;i<arr.length; i++){
		if(arr[i].str===item){
			if(callback){
				callback(true, arr[i]);
...
var validation = function(state, node, callback){
	loadRules(state, function(rules){
		...
		_err.gotAny = false;
		if(_err.loadCG.length>0 ||
			_err.loadModule.length>0 ||
			_err.loadCommonComponent.length>0 ||
			_err.properties.length>0
			){
			_err.gotAny = true;
		}
		callback(_err);
