...
var validLoadCGS = function(node, rules, callback){
    ...
    if(m0.length>0){
    	...
    	for(var i =0; i<elms.length; i++){
    		if(!isInLoadCGs(elms[i], rules.loadCG)){
    			_err.push({
    				title: `Sobran las dependencias: "${elms[i].properties[0].value.value}"`,
    				loc: elms[i].loc
    			});
    		}else{
    			_act.push({
    ...
    for(var t =0; t<rules.loadCG.length; t++){
		if(!isInArrCG(rules.loadCG[t].name, _act)){
			_err.push({
				title: `Falta de añadir el module: "${rules.loadCG[t].name}"`
			});
		}
	}
    callback(_err);
...
var validProperties = function(ast, rules, callback){
	var _err = [], _url = false, _params = false, _paramsValues = [], _sticky = false, _deepStateRedirect = false;
