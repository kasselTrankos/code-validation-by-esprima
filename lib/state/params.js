...
var plusValidTemplateURL = function(cntName, state){
	var _plus ={
		'GetCustomerServSupportEnquiry': ['viewIncident', 'selectInventoryType', 'showMLShippingOrderItem', 'showMLShippingOrder']
	};
	return (_plus[cntName] && _plus[cntName].lastIndexOf(state)>=0) ? true : false;
}
var validTemplateURL = function(node, name, stateName, cntName, callback){
	var _return = true;
	if(!(node.key &&
...
var _c =0;
var plusViews = function(cntName, state){
	var _plus = {
		'GetCustomerServSupportEnquiry': {
			'getCustomerServiceSuppEnquiry.showPartyRoleData':'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.searchCustomerContAndNot':'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.displayMblInsrCustmrConsult':'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.dActualCustomerConsumption': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.dResumeCustomerBill': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.displayQuotesDetail': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.showCustomerOrdersOH': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.scorecardMLShippingOrder': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.manageOrder': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.displaySPPOIConsultationNumber': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.undertakeCPTrendAnalysis': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.searchIncident': 'showOpcionMenuHijo',
			'getCustomerServiceSuppEnquiry.manageCSLIASLIByCustomer':'showOpcionMenuHijo'
		}
	}
	
	return (_plus[cntName] && _plus[cntName][state]) ? _plus[cntName][state] : '';
}
var getValidView = function(name, stateName, cntName){
	return plusViews (cntName, name) || stateName;
};
var validViews = function(node, name, stateName, cntName, callback){
	var _return = true;
	if(!(node.type==='Property' &&
		node.value.properties &&
		node.value.properties[0] &&
		node.value.properties[0].key.value==getValidView(name, stateName, cntName) &&
...
var params = function(node, loc, cntName, callback){
	...
	for(var i=0; i<node[1].properties.length; i++){
		...
		if(_name.split('.').length >1 && 
			...
			_existsView = true;
			validViews(node[1].properties[i], _name, _stateName, cntName,  function(e){
				if(e!==true){
					_err.push(e);
				}
			});
		...
		if(_name.split('.').length==1 && 
			...
			validTemplateURL(node[1].properties[i], _name, _stateName, cntName,  function(e){
				if(e!==true){
					_err.push(e);
				}
			});
	...
	if(_name.split('.').length==1 &&
		!_existsTemplateURL && !plusValidTemplateURL(cntName, _name)){
		(node[0]);
		_err.push({
